...
for nL = 1:p.nL_max
    for nChan = 1:p.nChan_max
        ...
        disp(cat(2,prefix,'>> import file ',p.mash_file{nL,nChan}));
        pushbutton_addTraces_Callback({p.annexpth,p.mash_file{nL,nChan}},...
            [],h_fig);
        ...
        % save project
        pushbutton_expProj_Callback({p.dumpdir,p.mash_file{nL,nChan}},[],...
            h_fig)
        
        % test ASCII files import 
        disp(cat(2,prefix,'>> import data set ',p.ascii_dir{nL,nChan}));
        set_TP_asciiImpOpt(p.asciiOpt{nChan,nL},h_fig);
...
set_VP_projOpt(p.projOpt{nL,nChan},p.wl(1:nL),h.pushbutton_editParam,...
    h_fig);

pushbutton_expProj_Callback({p.dumpdir,p.exp_coord1},[],h_fig);

% test coordinates import from trace file header
disp(cat(2,prefix,'test import of coordinates frome trace file headers',...
    '...'));
opt = p.asciiOpt{p.nChan,p.nL};
