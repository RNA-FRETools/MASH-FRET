...
if ~(isfield(h, 'figure_trsfOpt') && ishandle(h.figure_trsfOpt))
    ...
    ttstr_edit_start_i = cat(2,'<html><b>First column index</b> in file ',...
        'where<br>reference coordinates in channel %i<br>are written',...
        '</html>');
    ttstr_edit_iv_i = cat(2,'<html>Number of <b>columns to skip</b> in<br>',...
        'file to read reference coordinates<br>in channel %i</html>');
    ttstr_edit_stop_i = cat(2,'<html><b>Last column index</b> in file ',...
        'where<br>reference coordinates in channel %i<br>are written',...
        '</html>');
    ...
    h.trsfOpt.text_movDim = uicontrol('Style', 'text', 'String', ...
        'Movie dimensions (pixels): ', 'Units', 'pixels', ...
