function edit_thm_centreLow_Callback(obj, evd, h_fig)
...
maxVal = curr.thm_start{3}(gauss,5);
...
if ~(numel(val)==1 && ~isnan(val) && val<maxVal)
    setContPan(sprintf(['The lower limit of Gaussian center must ' ...
        'be higher than the starting value (%d)'],maxVal), 'error', h_fig);
