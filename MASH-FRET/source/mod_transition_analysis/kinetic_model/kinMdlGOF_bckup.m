...
%
% Calculate the goodness of fit of the model defined by the matrix of transition probabilities or transition numbers and eventually plot simulated dwell time histograms, sttae population and transition counts in comparison to experimental data.
%
...
% re-arrange experimental dwell times
mols = unique(dt_exp0(:,2));
dt_exp = [];
for m = mols'
    dt_exp_m = adjustDt(dt_exp0(dt_exp0(:,2)==m,:));
    if excl
        dt_exp_m([1,end],:) = [];
    end
...
ylbl = {};
for v1 = 1:J
    Nsim_degen = cat(1,Nsim_degen,zeros(1,J));
    for v2 = 1:J
        if v1==v2
            continue
        end
...
for j1 = 1:J
    ...
    cum_counts_exp{j1} = cumsum(hist(dt_exp_j1,bins{j1})');
    cum_counts_sim{j1} = cumsum(hist(dt_sim_j1,bins{j1})');
...
%         max_x = max_x(1);
        ...
        h_a = subplot(2,J+1,j1,'replace','parent',h_fig);
...
%         plot(h_a,bins{j1},fit_cum_sim,'-k'); % fit of simulation
        ...
        h_a = subplot(2,J+1,J+1+j1,'replace','parent',h_fig);
...
%         plot(h_a,bins{j1},fit_cmpl_sim,'-k'); % fit of simulation
        ...
        if j1==1
            ...
            h_a = subplot(2,J+1,J+1,'replace','parent',h_fig);
            b = bar(h_a,1:J,[popSim',popExp']);
            set(b(1),'facecolor','r');
            set(b(2),'facecolor','b');
            title(h_a,'state occupancy');
            
            % plot relative transition counts
            h_a = subplot(2,J+1,2*(J+1),'replace','parent',h_fig);
