...
if ~isempty(p.proj)
    ...
    a{1} = prm.plot{1}(1,1); % TDP binning
    a{2} = prm.plot{1}(1,[2 3]); % TDP limits
    a{3} = p.proj{proj}.frame_rate; % frame rate
    a{4} = prm.plot{1}(4,[1,2]); % transition count/re-arrange sequences
    ...
    if ~isempty(TDP)
        ...
        plot_prm{1} = a{2}; % TDP limits
        plot_prm{2} = a{1}; % TDP binning
