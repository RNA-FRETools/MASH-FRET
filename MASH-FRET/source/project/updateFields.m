    ...
    switch p.noiseType
        case 'poiss' % Poisson or P-model from Börner et al. 2017
            ...
            set([h.text_camNoise_01 h.edit_camNoise_01  ...
                h.text_camNoise_02 h.edit_camNoise_02], 'Enable', 'on');
            
            set([h.text_camNoise_03 h.edit_camNoise_03 ...
            ...
            %set(h.text_camNoise_01, 'Style', 'push', 'enable', 'off', 'String', '<HTML>&omega;</HTML>');
            set(h.text_camNoise_01, 'Style', 'text', 'String', [char(hex2dec('3BC')),'_ic,dark:']); % dark counts or offset in ic
            set(h.text_camNoise_02, 'Style', 'text', 'String', [char(hex2dec('3B7')),':']); % total detection efficiency
            ...
            set(h.edit_camNoise_01, 'String', ...
                num2str(p.camNoise(ind,1)), 'TooltipString', ...
                'Dark counts or camera offset (ic/frame)');
        ...
        case 'norm' % Gaussian, Normal or N-model from Börner et al. 2017
            ...
            set(h.edit_camNoise_05, 'Enable', 'inactive');
            
            [o, mu_rho_stat] = Saturation(p.bitnr);
            
            set(h.text_camNoise_01, 'String', 'K:'); % Overall system gain
            set(h.text_camNoise_02, 'String', [char(hex2dec('3C3')),'_d:']); % Read-out noise width
            set(h.text_camNoise_03, 'String', [char(hex2dec('3BC')),'_ic,dark:']); % Dark counts or offset
            set(h.text_camNoise_04, 'String', [char(hex2dec('3C3')),'_q:']); % Analog-to-digital noise width
            set(h.text_camNoise_05, 'String', 'sat:', 'TooltipString', ...
                'Max. number of photons that hit the a single pixel area');
            set(h.text_camNoise_06, 'String', [char(hex2dec('3B7')),':']); % total detection efficiency

            set(h.edit_camNoise_01, 'String', ...
                num2str(p.camNoise(ind,1)), ...
                'TooltipString', 'Overall system gain (PC/electron/frame)'); % K
            set(h.edit_camNoise_02, 'String', ...
                num2str(p.camNoise(ind,2)), 'TooltipString', ...
                ['Noise standard deviation of the  ' ...
                'contribution due to read-out noise (PC/frame)']); % s_d
            set(h.edit_camNoise_03, 'String', ...
                num2str(p.camNoise(ind,3)), ...
                'TooltipString', ...
                'Mean of the Gaussian noise contribution (EC/frame)'); % mu_y.dark
            set(h.edit_camNoise_04, 'String', ...
                num2str(p.camNoise(ind,4)), 'TooltipString', ...
                ['Noise standard deviaton of the contribution due to' ...
                ' analog-to-digital conversion noise (PC/frame)']); % s_q
            set(h.edit_camNoise_05, 'String', ...
                num2str(mu_rho_stat), ...
                'TooltipString', 'Saturation image count number per pixel and frame'); % mu_r.stat
            set(h.edit_camNoise_06, 'String', ...
                num2str(p.camNoise(ind,6)), ...
                'TooltipString', 'Total Quantum Efficiency'); % eta
        ...
        case 'user' % User defined or NExpN-model from Börner et al. 2017
            ...
            set(h.text_camNoise_03, 'String', [char(hex2dec('3C4')),'_CIC:']); % CIC decay constant
            ...
            set(h.edit_camNoise_01, 'String', ...
                num2str(p.camNoise(ind,1)), 'TooltipString', ...
                'Camera offset (EC/frame)'); % Dark count or offset
            set(h.edit_camNoise_02, 'String', ...
                num2str(p.camNoise(ind,2)), 'TooltipString', ...
                'CIC exponential tail contribution'); % A_CIC
            set(h.edit_camNoise_03, 'String', num2str(p.camNoise(ind,3)), ...
                'TooltipString', 'CIC decay constant (EC/frame)'); % tau_CIC
            set(h.edit_camNoise_04, 'String', num2str(p.camNoise(ind,4)), ...
                'TooltipString', ...
                'Initial width of the camera noise (EC/frame)');
        ...
        case 'none' 
            ...
            %set(h.text_camNoise_01, 'String', 'mu.d:');

            set(h.edit_camNoise_01, 'String', ...
                num2str(p.camNoise(ind,1)), 'TooltipString', ...
                'Camera Offset (ic/frame)');
        ...
        case 'hirsch'
            ...
            set(h.text_camNoise_01, 'String', 'g:'); % camera amplification gain
            set(h.text_camNoise_02, 'String', [char(hex2dec('3C3')),'_d:']); % read-out noise width
            set(h.text_camNoise_03, 'String', [char(hex2dec('3BC')),'_ic,dark']); % Dark count or camera offset
            set(h.text_camNoise_04, 'String', 'CIC:'); % CIC contribution
            set(h.text_camNoise_05, 'String', 's:'); % amplifier sensitivity
            set(h.text_camNoise_06, 'String', [char(hex2dec('3B7')),':']); % total detection efficiency

            set(h.edit_camNoise_01, 'String', ...
                num2str(p.camNoise(ind,1)), ...
                'TooltipString', 'System gain (pc/electron/frame)'); % g
            set(h.edit_camNoise_02, 'String', ...
                num2str(p.camNoise(ind,2)), 'TooltipString', ...
                ['Noise standard deviation of the read-out noise ' ...
                '(PC/frame)']); % s_d
            set(h.edit_camNoise_03, 'String', ...
                num2str(p.camNoise(ind,3)), ...
                'TooltipString', ...
                'Dark counts or camera offset (ic/frame)'); % mu_y.dark
            set(h.edit_camNoise_04, 'String', ...
                num2str(p.camNoise(ind,4)), 'TooltipString', ...
                ['CIC contribution (pc/frame)' ...
                '(PC/frame)']); % CIC
            set(h.edit_camNoise_05, 'String', ...
                num2str(p.camNoise(ind,5)), 'TooltipString', ...
                'Amplifier Sensitivity or analog-to-digital factor (ic/pc/frame)'); % s
            set(h.edit_camNoise_06, 'String', ...
                num2str(p.camNoise(ind,6)), ...
                'TooltipString', 'Total detection efficiency'); % eta
    end
...
if strcmp(opt, 'TDP') || strcmp(opt, 'all')
    ...
    if ~isempty(p.proj)
        ...
        set([h.togglebutton_TDPkmean h.togglebutton_TDPgauss], ...
            'Value', 0, 'FontWeight', 'normal');
        setProp(get(h.uipanel_TDPana, 'Children'), 'Enable', 'off');
