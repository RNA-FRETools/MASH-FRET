...
str3 = 'Export';
ttstr0 = wrapHtmlTooltipString('<b>Axis scale</b> used to calculate and plot survival time histograms: <b>linear-linear</b> (x and y axis in linear scale), <b>log-linear</b> (x axis in log scale and y axis in linear scale), <b>linear-log</b> (x axis in linear scale and y axis in log scale), and <b>log-log</b> (x and y axis in log scale).');
...
wtxt = getUItextWidth(str0,p.fntun,p.fntsz1,'normal',p.tbl);
wpop = getUItextWidth(str1{1},p.fntun,p.fntsz1,'normal',p.tbl)+p.warr;
wbut1 = getUItextWidth(str3,p.fntun,p.fntsz1,'normal',p.tbl)+p.wbrd;
wpop = max([wpop,wtxt,wbut1]);
wax = (wfig-3*p.mg)/2;
hax = hfig-2*p.mg;
...
h.text_bleachscale = uicontrol('style','text','parent',fig,'units',p.posun,...
    ...
    [x,y,wpop,p.htxt0],'backgroundcolor',ax.Color);

y = y-p.hpop0;

h.popup_bleachscale = uicontrol('style','popupmenu','parent',fig,...
    'units',p.posun,'fontunits',p.fntun,'fontsize',p.fntsz1,'position',...
    [x,y,wpop,p.hpop0],'string',str1,'tooltipstring',ttstr0,'callback',...
...
h.push_bleachexport = uicontrol('style','pushbutton','parent',fig,'units',...
    ...
    [x,y,wpop,p.hedit0],'string',str3,'tooltipstring',ttstr2,'callback',...
...
h.popup_blinkscale = uicontrol('style','popupmenu','parent',fig,...
    'units',p.posun,'fontunits',p.fntun,'fontsize',p.fntsz1,'position',...
    [x,y,wpop,p.hpop0],'string',str1,'tooltipstring',ttstr3,'callback',...
...
h.push_blinkexport = uicontrol('style','pushbutton','parent',fig,'units',...
    p.posun,'fontunits',p.fntun,'fontsize',p.fntsz1,'position',...
    [x,y,wpop,p.hedit0],'string',str3,'tooltipstring',ttstr5,'callback',...
