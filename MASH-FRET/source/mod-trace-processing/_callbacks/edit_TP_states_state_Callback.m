function edit_TP_states_state_Callback(obj, evd, h_fig)
...
if ~(~isempty(val) && numel(val) == 1 && ~isnan(val))
    ...
    updateActPan('State value must be a number.', h_fig, 'error');
    return
end
...
p.proj{proj}.TP.curr{mol}{4}{4}(1,state,chan_in) = val;
