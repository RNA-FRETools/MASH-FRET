...
for l = 1:nExc
    for c = 1:nChan
        ...
        if ~prm_bg{1}(l,c,1)
            ...
            if size(prm_bg{3}{l,c},1)==1
                appmethod = 1;
            else
                appmethod = prm_bg{2}(l,c);
            end
            bgInt = prm_bg{3}{l,c}(appmethod,3);
            if perSec
                bgInt = bgInt/splt;
            end
            
            if prm_bg{1}(l,c,2)
                str_bg = cat(2,str_bg,', averaged BG intensity: ',...
                    num2str(bgInt), ' (method: dynamic "',...
                    str_meth_bg{prm_bg{2}(l,c)},'"');
            else
            ...
            switch prm_bg{2}(l,c)
                ...
                case 2 % darkest
                    str_bg = cat(2,str_bg,', sub-image dimensions: ',...
                        num2str(prm_bg{3}{l,c}(appmethod,2)),' px)\n');
                    
                case 3 % mean
                    str_bg = cat(2,str_bg,', tolerance cutoff: ',...
                        num2str(prm_bg{3}{l,c}(appmethod,1)),...
                ...
                case 6 % dark trace
                    if prm_bg{1}(l,c,2)
                        str_bg = cat(2,str_bg,', running average window ',...
                            'size: ',num2str(prm_bg{3}{l,c}(appmethod,1)));
                    end
                    str_bg = cat(2,str_bg,', sub-image dimensions: ',...
                        num2str(prm_bg{3}{l,c}(appmethod,2)),' px',...
                        ', dark coordinates: (',...
                        num2str(prm_bg{3}{l,c}(appmethod,4)),',',...
                        num2str(prm_bg{3}{l,c}(appmethod,5)),'))\n');
