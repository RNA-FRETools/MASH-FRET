...
if size(p_proj.prm{n},2)>=5 && size(p_proj.prm{n}{5},2)>=2 && ...
        iscell(p_proj.prm{n}{5}{1})
    newBtPrm = zeros(nChan,nChan-1);
    for c = 1:nChan
        bts = zeros(nExc,nChan-1);
        for l = 1:nExc
            bts(l,:) = p_proj.prm{n}{5}{1}{l,c};
        end
        newBtPrm(c,:) = sum(bts,1);
    end
    p_proj.prm{n}{5}{1} = newBtPrm;
end
...
if size(p_proj.prm{n},2)>=5 && size(p_proj.prm{n}{5},2)>=2 && ...
    ...
    for c = 1:nChan
        ...
        for l = 1:nExc
            if l ~= l0
                exc_dir = exc_dir+1;
                val = find(p_proj.prm{n}{5}{2}{l,c}~=0);
                if isempty(val)
                    newDePrm(exc_dir,c) = 0;
                else
                    newDePrm(exc_dir,c) = p_proj.prm{n}{5}{2}{l,c}(val(1));
                end
            end
        end
    end
...
% regression
if size(p_proj.prm{n},2)==5 && size(p_proj.prm{n}{5},2)==5
    factPrm =[p_proj.prm{n}{5}(3:end),p_proj.def.mol{6}{4}];
    p_proj.prm{n} = cat(2,p_proj.prm{n},{factPrm});
    if n==1
        p_proj.fix{4} = p_proj.prm{n}{5}(1:2);
    end
    p_proj.prm{n}{5} = [];
...
if size(p_proj.prm{n},2)>=6 
    % added by MH, 13.1.2020: down compatibility by adding beta factors
    if size(p_proj.prm{n}{6},2)>=1 && size(p_proj.prm{n}{6}{1},1)==1
        p_proj.prm{n}{6}{1} = cat(1,p_proj.prm{n}{6}{1},...
            ones(1,size(p_proj.prm{n}{6}{1},2)));
    end
    
    % added by MH, 14.1.2020: recover gamma/beta correction method
    if size(p_proj.prm{n}{6},2)>=2 && size(p_proj.prm{n}{6}{2},2)<nFRET
        p_proj.prm{n}{6}{2} = repmat(p_proj.prm{n}{6}{2}(1),1,nFRET);
    end
...
if size(p_proj.prm{n},2)>=4 && size(p_proj.prm{n}{4},2)>=2
    ...
    % reshape old format
    if size(p_proj.prm{n}{4}{2},2)==8
        for j = 1:size(p_proj.prm{n}{4}{2},3)
            p_proj.prm{n}{4}{2}(1,1:6,j) = ...
                p_proj.prm{n}{4}{2}(1,[2,1,5,8,3,4],j);
            p_proj.prm{n}{4}{2}(2,1:6,j) = ...
                p_proj.prm{n}{4}{2}(2,[1,2,5,8,3,4],j);
            p_proj.prm{n}{4}{2}(3,1:6,j) = ...
                p_proj.prm{n}{4}{2}(3,[1,2,5,8,3,4],j);
            p_proj.prm{n}{4}{2}(4,1:6,j) = ...
                p_proj.prm{n}{4}{2}(4,[5,6,7,8,3,4],j);
            p_proj.prm{n}{4}{2}(5,1:6,j) = ...
                p_proj.prm{n}{4}{2}(5,[2,1,5,8,3,4],j);
        end
        p_proj.prm{n}{4}{2} = ...
...
if p_proj.is_movie && size(p_proj.prm{n},2)>=3
    for l = 1:nExc
        for c = 1:nChan
            if size(p_proj.prm{n}{3},2)>=4 && p_proj.prm{n}{3}(4)>0
                p_proj.prm{n}{3}{3}{l,c}(p_proj.prm{n}{3}{3}{l,c}(:,2)'==0,2) = ...
                    p_proj.prm{n}{3}(4);
            elseif p_proj.fix{1}(2)>0
                p_proj.prm{n}{3}{3}{l,c}(p_proj.prm{n}{3}{3}{l,c}(:,2)'==0,2) = ...
                    p_proj.fix{1}(2);
            else
                p_proj.prm{n}{3}{3}{l,c}(p_proj.prm{n}{3}{3}{l,c}(:,2)'==0,2) = ...
            ...
            if p_proj.prm{n}{3}{3}{l,c}(5,1)>1
                p_proj.prm{n}{3}{3}{l,c}(5,1) = 0.5;
            end
            % for median, the parameter should be 1 or 2.
            if ~(p_proj.prm{n}{3}{3}{l,c}(7,1)==1 || ...
                    p_proj.prm{n}{3}{3}{l,c}(7,1)==2)
                p_proj.prm{n}{3}{3}{l,c}(7,1) = 2;
            end
        end
    end
    if size(p_proj.prm{n}{3},2)>=4
        p_proj.prm{n}{3}(4) = [];
    end
end
...
if size(p_proj.prm{n},2)>=2 && ...
        size(p_proj.prm{n}{2}{2},1)~=(nFRET+nS+2+nExc*nChan)
    sz = size(p_proj.prm{n}{2}{2},1);
    diffsz = sz-(nFRET+nS+2+nExc*nChan);
    id_s = (nFRET+1):(sz-2-nExc*nChan);
    id_excl = id_s((end-diffsz+1):end);
    if ~isempty(id_excl)
        p_proj.prm{n}{2}{2}(id_excl,:) = []; % re-arrange photobleaching
        p_proj.prm{n}{4}{2}(:,:,id_excl) = []; % rearrange DTA (param)
        p_proj.prm{n}{4}{3}(id_excl,:) = []; % rearrange DTA (states)
