function ud_TTprojPrm(h_fig)
h = guidata(h_fig);
p = h.param.ttPr;

if ~isempty(p.proj)
    isMov = p.proj{p.curr_proj}.is_movie;
    isCoord = p.proj{p.curr_proj}.is_coord;
    nC = p.proj{p.curr_proj}.nb_channel;
    labels = p.proj{p.curr_proj}.labels;
    FRET = p.proj{p.curr_proj}.FRET;
    S = p.proj{p.curr_proj}.S;
    nFRET = size(FRET,1);
    nS = size(S,1);
    exc = p.proj{p.curr_proj}.excitations;
    nExc = p.proj{p.curr_proj}.nb_excitations;
    incl = p.proj{p.curr_proj}.coord_incl;
    p_fix = p.proj{p.curr_proj}.fix;
    nMol = size(incl,2);
    perSec = p_fix{2}(4);
    perPix = p_fix{2}(5);
    
    setProp(get(h.uipanel_TTproc, 'Children'), 'Enable', 'on');
    set(h.text_molTot, 'String', ['total: ' num2str(nMol) ' molecules']);
    set(h.listbox_traceSet, 'Max', 2, 'Min', 0);

    if ~isCoord
        setProp([h.text_subImg_exc h.popupmenu_subImg_exc ...
            h.checkbox_refocus h.text_brightness ...
            h.slider_brightness h.text_contrast h.slider_contrast ...
            h.text_subImg_dim h.edit_subImg_dim h.uitable_molCoord ...
            h.text_xDark h.edit_xDark h.text_yDark h.edit_yDark ...
            h.checkbox_autoDark h.pushbutton_showDark ...
            h.edit_trBgCorrParam_01], 'Visible', 'off');
        set(h.popupmenu_trBgCorr, 'Value', 1, 'String', {'Manual'});

    elseif ~isMov
        set(h.uitable_molCoord, 'Visible', 'on', 'Enable', 'inactive');
        setProp([ h.text_subImg_exc h.popupmenu_subImg_exc ...
            h.checkbox_refocus h.text_brightness h.slider_brightness ...
            h.text_contrast h.slider_contrast h.text_subImg_dim ...
            h.edit_subImg_dim h.text_xDark h.edit_xDark ...
            h.text_yDark h.edit_yDark h.checkbox_autoDark ...
            h.pushbutton_showDark h.edit_trBgCorrParam_01], 'Visible', ...
            'off');
        set(h.popupmenu_trBgCorr, 'Value', 1, 'String', {'Manual'});

    else
        set(h.uitable_molCoord, 'Visible', 'on', 'Enable', 'on');
        setProp([h.text_subImg_exc h.popupmenu_subImg_exc ...
            h.checkbox_refocus h.text_brightness h.slider_brightness ...
            h.text_contrast h.slider_contrast h.text_subImg_dim ...
            h.edit_subImg_dim h.text_xDark h.edit_xDark ...
            h.text_yDark h.edit_yDark h.checkbox_autoDark ...
            h.pushbutton_showDark h.edit_trBgCorrParam_01], 'Visible', ...
            'on');
        set(h.popupmenu_trBgCorr, 'Value', 1, 'String', {'Manual', ...
            '<N median values>', 'Mean value', 'Most frequent value', ...
            'Histothresh', 'Dark trace', 'Median value'});
    end

    if nC == 1
        setProp([h.text_bt h.popupmenu_bt h.edit_bt], 'Visible', 'off');
    else
        setProp([h.text_bt h.popupmenu_bt h.edit_bt], 'Visible', 'on');
    end
    if (nFRET + nS) == 0
        set([h.radiobutton_DTA2bottom h.radiobutton_DTA2top ...
            h.radiobutton_DTA2all h.popupmenu_plotBottom ...
            h.text_plotBottom], 'Visible', 'off');
        set(h.text_topAxes, 'String', 'channel');
    else
        set(h.popupmenu_plotBottom, 'String', getStrPop('plot_botChan', ...
            {FRET S exc p.proj{p.curr_proj}.colours labels}), 'Value', ...
            p_fix{2}(3));
        set([h.radiobutton_DTA2bottom h.radiobutton_DTA2top ...
            h.radiobutton_DTA2all h.popupmenu_plotBottom ...
            h.text_plotBottom], 'Visible', 'on');
        set(h.text_topAxes, 'String', 'top axes:');
    end
    if nExc == 1
        set([h.popupmenu_excDirExc h.text_dirExc h.edit_dirExc], ...
            'Visible', 'off');
    else
        set([h.popupmenu_excDirExc h.text_dirExc h.edit_dirExc], ...
            'Visible', 'on');
    end
    
    set([h.popupmenu_plotTop h.popupmenu_bleachChan ...
        h.popupmenu_corr_chan h.popupmenu_bt h.popupmenu_DTAchannel ...
        h.popupmenu_trBgCorr_chan h.popupmenu_subImg_exc ...
        h.popupmenu_trBgCorr_exc h.popupmenu_corr_exc ...
        h.popupmenu_ttPlotExc h.popupmenu_excDirExc], 'Value', 1);

    set(h.popupmenu_trBgCorr_chan, 'String', getStrPop('chan', ...
        {labels p_fix{3}(5) p.proj{p.curr_proj}.colours{1}}));
    set(h.popupmenu_corr_chan, 'String', getStrPop('chan', ...
        {labels p_fix{3}(1) p.proj{p.curr_proj}.colours{1}}));
    set(h.popupmenu_bt, 'String', getStrPop('bt_chan', ...
        {labels p_fix{3}(2) p_fix{3}(1) p.proj{p.curr_proj}.colours{1}}));
    set(h.popupmenu_DTAchannel, 'String', getStrPop('DTA_chan', ...
        {labels FRET S exc p.proj{p.curr_proj}.colours}));
    set(h.popupmenu_plotTop, 'String', getStrPop('plot_topChan', ...
        {labels p_fix{2}(1) p.proj{p.curr_proj}.colours{1}}));
    set(h.popupmenu_bleachChan, 'String', getStrPop('bleach_chan', ...
        {labels FRET S exc p.proj{p.curr_proj}.colours}));

    
    set([h.popupmenu_subImg_exc h.popupmenu_trBgCorr_exc ...
        h.popupmenu_corr_exc], 'String', getStrPop('exc', exc));
    set(h.popupmenu_excDirExc, 'String', getStrPop('dir_exc', ...
        {exc, p_fix{3}(1), p_fix{3}(2), p.proj{p.curr_proj}.colours{1}}));
    set(h.popupmenu_ttPlotExc, 'String', getStrPop('plot_exc', exc));
    
    set(h.popupmenu_plotTop, 'Value', p_fix{2}(2));
    set(h.popupmenu_bleachChan, 'Value', 1);
    set(h.popupmenu_corr_chan, 'Value', p_fix{3}(2));
    set(h.popupmenu_bt, 'Value', p_fix{3}(3));
    set(h.popupmenu_DTAchannel, 'Value', p_fix{3}(4));
    set(h.popupmenu_trBgCorr_chan, 'Value', p_fix{3}(6));
    set(h.popupmenu_subImg_exc, 'Value', p_fix{1}(1));
    set(h.popupmenu_trBgCorr_exc, 'Value', p_fix{3}(5));
    set(h.popupmenu_corr_exc, 'Value', p_fix{3}(1));
    set(h.popupmenu_ttPlotExc, 'Value', p_fix{2}(1));
    set(h.popupmenu_excDirExc, 'Value', p_fix{3}(7));
    
    set(h.checkbox_ttPerSec, 'Value', perSec);
    set(h.checkbox_ttAveInt, 'Value', perPix);
    
    if isfield(h, 'axes_subImg')
        for i = 1:numel(h.axes_subImg)
            if ishandle(h.axes_subImg(i))
                delete(h.axes_subImg(i));
            end
        end
        h = rmfield(h, 'axes_subImg');
        refresh;
    end
    
    if isCoord && isMov
        posPanel = get(h.listbox_traceSet, 'Position');
        units = get(h.listbox_traceSet, 'Units');
        posList = get(h.listbox_traceSet, 'Position');
        posBg = get(h.uipanel_traceBgCorr, 'Position');
        mg = posList(1);
        wImg = (posBg(1)-(posPanel(1)+posPanel(3))-(nC+1)*mg)/nC;
        hImg = posPanel(4);
        yNext = posPanel(2);
        xNext = posPanel(1) + posPanel(3) + mg;
        c = (linspace(0, 1, 50))';
        cmap = [c c c];
        for i = 1:nC
            h.axes_subImg(i) = axes('Parent', h.uipanel_TTproc, ...
                'Units', units, 'Position', [xNext yNext wImg hImg], ...
                'FontUnits', 'pixels', 'FontSize', 11, 'Visible', 'off');
            colormap(h.axes_subImg(i), cmap);
            xNext = xNext + wImg + mg;
        end
        if ~(isfield(p.proj{p.curr_proj}, 'aveImg') && ...
                size(p.proj{p.curr_proj}.aveImg,2) == nExc)
            param.stop = p.proj{p.curr_proj}.movie_dat{3};
            param.iv = nExc;
            param.file = p.proj{p.curr_proj}.movie_file;
            param.extra = p.proj{p.curr_proj}.movie_dat; 
            for l = 1:nExc
                param.start = l;
                [img ok] = createAveIm(param, 0, h_fig);
                if ~ok
                    return;
                end
                p.proj{p.curr_proj}.aveImg{l} = img;
                h.param.ttPr = p;
            end
        end
    end
else
    if isfield(h, 'axes_subImg')
        for i = 1:numel(h.axes_subImg)
            if ishandle(h.axes_subImg(i))
                delete(h.axes_subImg(i));
            end
        end
        h = rmfield(h, 'axes_subImg');
        refresh;
    end
    setProp(get(h.uipanel_TTproc, 'Children'), 'Enable', 'off');
    setProp([h.pushbutton_traceImpOpt h.pushbutton_addTraces], ...
        'Enable', 'on');
end
cla(h.axes_top);
cla(h.axes_topRight);
cla(h.axes_bottom);
cla(h.axes_bottomRight);
set([h.axes_top, h.axes_topRight, h.axes_bottom, h.axes_bottomRight], ...
    'Visible', 'off');

guidata(h_fig, h);
